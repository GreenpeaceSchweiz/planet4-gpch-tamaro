...
export default function Edit( { attributes, setAttributes } ) {
	...
	const addCustomField = () => {
		if ( customFields.length >= MAX_FIELDS ) {
			return;
		}
	...
	const removeCustomField = ( index ) => {
		if ( customFields.length <= MIN_FIELDS ) {
			return;
		}
				...
				<PanelBody title="Amount Descriptions" initialOpen={ false }>
					...
					{ useAmountDescriptions && (
						<>
							...
							<PanelRow>
								<p>
									<strong>Onetime Amounts</strong>
								</p>
							</PanelRow>

							{ amountsOnetime &&
								amountsOnetime.split( ',' ).map( ( amount ) => {
							...
							<PanelRow>
								<p>
									<strong>Recurring Monthly Amounts</strong>
								</p>
							</PanelRow>
