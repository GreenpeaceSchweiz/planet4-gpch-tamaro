...
const CustomField = ( { field, index, onChange, onRemove, canRemove } ) => {
	...
	const titleText =
		fieldLabel && fieldLabel.trim().length
			? fieldLabel.trim()
			: sprintf(
					// translators: %d represents the number of the custom field
					__( 'Custom field %d', 'planet4-gpch-tamaro' ),
					index + 1
			  );
		...
		>
			...
			{ canBeRequired && (
				<CheckboxControl
					...
					help={ __(
						'The form can not be submitted without filling in this field.',
						'planet4-gpch-tamaro'
					) }
				/>
			) }
			...
			{ canBePreselected && (
				<CheckboxControl
					...
					onChange={ ( val ) =>
						updateField( { isPreselected: val } )
					}
					help={ __(
						'Render this checkbox as selected by default in the form.',
						'planet4-gpch-tamaro'
					) }
				/>
			) }
				...
				<>
					{ normalizedOptions.map( ( option, optionIndex ) => (
						...
						>
							<TextControl
								label={ sprintf(
									// translators: %d represents the number of the option
					...
					>
						<Button
							variant="secondary"
							onClick={ addOption }
							disabled={ normalizedOptions.length >= MAX_OPTIONS }
						>
						...
						<span>
							{ sprintf(
								// translators: %1$d represents the number of the custom field, %2$d represents the maximum number of options
								__(
									'%1$d / %2$d options',
									'planet4-gpch-tamaro'
								),
